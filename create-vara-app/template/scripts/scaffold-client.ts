...
const DEFAULT_IDL_REL_PATH = "src/assets/demo.idl";

function scaffoldCommand(idlRelPath: string, executable = "npx tsx"): string {
  const idlArg = idlRelPath === DEFAULT_IDL_REL_PATH ? "" : ` ${idlRelPath}`;
  return `${executable} ../scripts/scaffold-client.ts${idlArg}`;
}
...
function generateSailsClient(
  ...
  lines.push(
    `// THIS FILE IS AUTO-GENERATED by scripts/scaffold-client.ts`,
    `// Regenerate: cd frontend && ${scaffoldCommand(idlRelPath)}`,
...
function generatedUiHeader(idlRelPath: string): string {
  return [
    `// THIS FILE IS AUTO-GENERATED by scripts/scaffold-client.ts`,
    `// Regenerate: cd frontend && ${scaffoldCommand(idlRelPath, "./node_modules/.bin/tsx")}`,
...
function generateActionsPanel(
  ...
  L.push(generatedUiHeader(idlRelPath));
  ...
  for (const cmd of commands) {
    ...
    const params = getParams(cmd);
    const fnName = txFnName(cmd.name);
    const paramVars = params.map((p) => {
      const stateVar = `${prefix}${p.name.charAt(0).toUpperCase() + p.name.slice(1)}`;
      return isBigNumeric(p.typeLabel) ? `BigInt(${stateVar})` : stateVar;
    });
...
function generateStatePanel(
  ...
  L.push(`function MetricRow({ value, label, accentColor = "border-emerald-500/40" }: { value: string | number | bigint; label: string; accentColor?: string }) {`);
  L.push(`  return (`);
